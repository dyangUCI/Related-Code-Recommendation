...
We further cluster those candidate related methods identified in the previous step based on their token similarity, in order to detect common co-occurring code fragments. Given each candidate related method, we retrieve its token list again and computes its similarity to other candidates which are from different GitHub files. Each candidate will serve as the center of a cluster, we browse among other candidates from other files and add similar candidates to the current cluster.
